...
// Status is edited on its own so the shelf can offer a one-click "send to
// repair" without opening the whole form. Changing status never touches
// custody: an item in for repair is still assigned to its holder.
export async function setEquipmentStatus(id: string, status: EquipmentStatus): Promise<Result> {
  const admin = await requireAdmin();
  if (!EQUIPMENT_STATUSES.includes(status)) return { ok: false, error: "Invalid status." };
  ...
  await recordAudit({ table: "equipment", recordId: id, operation: "update", actor: admin.email, newData: { status } });
  refresh();
  return { ok: true };
}
