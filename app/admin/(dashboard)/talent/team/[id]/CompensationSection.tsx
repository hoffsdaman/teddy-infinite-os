...
import { type SalaryRow } from "@/lib/admin/compensation-shared";
...
export function CompensationSection({
  ...
  function submit() {
    setBanner(null);
    const salaryAudCents = Math.round(Number(aud.replace(/,/g, "").trim()) * 100);
  ...
  return (
    <div className="admin-card admin-section-card">
      ...
      <dl className="admin-kv">
        <dt>Salary (AUD)</dt>
        <dd className="admin-cell-mono">{current ? formatCents(current.salaryAudCents, "aud") : "—"}</dd>
        <dt>Since</dt>
      ...
      {open && (
        <div className="admin-form u-mt-3">
          <div className="admin-field">
            <label className="admin-label">Salary (AUD / month)</label>
            <input
              ...
              {history.map((h) => (
                <tr key={h.id}>
                  ...
                  <td className="admin-cell-mono u-right">
                    {formatCents(h.salaryAudCents, "aud")}
                  </td>
