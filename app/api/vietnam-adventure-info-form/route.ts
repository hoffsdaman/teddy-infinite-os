...
const FROM = 'TeddyBed OS Adventures <hello@teddybed.com.au>'
const DEFAULT_NOTIFY = 'hello@teddybed.com.au'
        ...
        await resend.emails.send({
          ...
          html: `
            ...
            <table style="border-collapse:collapse;font-family:sans-serif;font-size:15px;margin-bottom:16px">
              <tr><td style="padding:6px 16px 6px 0;color:${P.inkBody}">Family</td><td><strong>${esc(family_name)}</strong></td></tr>
              <tr><td style="padding:6px 16px 6px 0;color:${P.inkBody}">Contact</td><td>${esc(contact_name)}</td></tr>
              <tr><td style="padding:6px 16px 6px 0;color:${P.inkBody}">Email</td><td><a href="mailto:${esc(contact_email)}">${esc(contact_email)}</a></td></tr>
              <tr><td style="padding:6px 16px 6px 0;color:${P.inkBody}">Passport photos</td><td>${totalPhotos}</td></tr>
            </table>
            ...
            <p style="color:${P.muted};font-size:12px;margin-top:16px">Passport links expire after 7 days. Photos are auto-deleted within 30 days of the trip.</p>
          `,
        })
      } catch (mailErr) {
