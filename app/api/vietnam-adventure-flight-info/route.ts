        ...
        await resend.emails.send({
          ...
          html: `
            ...
            <table style="border-collapse:collapse;font-family:sans-serif;font-size:15px;margin-bottom:20px">
              <tr><td style="padding:6px 16px 6px 0;color:${P.inkBody};width:160px">Family</td><td><strong>${esc(family_name)}</strong></td></tr>
              <tr><td style="padding:6px 16px 6px 0;color:${P.inkBody}">Email</td><td><a href="mailto:${esc(contact_email)}">${esc(contact_email)}</a></td></tr>
            </table>
            <h3 style="font-family:sans-serif;font-size:13px;text-transform:uppercase;color:${P.inkBody};letter-spacing:.05em;margin-bottom:8px">Arriving in Hanoi</h3>
            <table style="border-collapse:collapse;font-family:sans-serif;font-size:14px;margin-bottom:20px">
              <tr><td style="padding:4px 16px 4px 0;color:${P.inkBody};width:160px">Flight</td><td>${esc(body.inbound_flight || '—')}</td></tr>
              <tr><td style="padding:4px 16px 4px 0;color:${P.inkBody}">Arrival</td><td>${fmt(body.inbound_arr_date, body.inbound_arr_time)}</td></tr>
            </table>
            <h3 style="font-family:sans-serif;font-size:13px;text-transform:uppercase;color:${P.inkBody};letter-spacing:.05em;margin-bottom:8px">Departing from Vietnam</h3>
            <table style="border-collapse:collapse;font-family:sans-serif;font-size:14px">
              <tr><td style="padding:4px 16px 4px 0;color:${P.inkBody};width:160px">Flight</td><td>${esc(body.outbound_flight || '—')}</td></tr>
              <tr><td style="padding:4px 16px 4px 0;color:${P.inkBody}">Departure</td><td>${fmt(body.outbound_dep_date, body.outbound_dep_time)}</td></tr>
            </table>
            ${body.notes ? `<h3 style="font-family:sans-serif;font-size:13px;text-transform:uppercase;color:${P.inkBody};letter-spacing:.05em;margin-bottom:8px">Comments / Requests</h3><p style="font-family:sans-serif;font-size:14px;line-height:1.6;white-space:pre-wrap">${esc(body.notes)}</p>` : ''}
          `,
        })
      } catch (mailErr) {
