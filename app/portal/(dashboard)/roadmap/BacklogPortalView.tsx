...
export function BacklogPortalView({
  ...
  function renderItem(it: BacklogItem, index: number) {
    ...
    return (
      <Draggable draggableId={it.id} index={index} key={it.id} isDragDisabled={!canPrioritize}>
        {(provided, snapshot) => (
          <div
            className={`admin-pbacklog-item${snapshot.isDragging ? " dragging" : ""}`}
            ref={provided.innerRef}
            {...provided.draggableProps}
          >
            <span
              className="admin-pbacklog-handle"
            ...
            <div className="admin-pbacklog-main">
              <div className="admin-pbacklog-item-top">
                {it.ref && <span className="admin-pbacklog-ref">{it.ref}</span>}
                <span className="admin-pbacklog-title">{it.title}</span>
                <span className="admin-pbacklog-pills">
                  {canPrioritize ? (
                    BACKLOG_PRIORITIES.map((p) => (
                      <button
                        key={p}
                        type="button"
                        className={`admin-pbacklog-pill${eff === p ? ` on-${p}` : ""}`}
                        disabled={pending}
                        onClick={() => run(() => setMyPriority(it.id, it.client_priority === p ? null : p))}
                        title={it.client_priority ? "Your priority" : "TeddyBed OS proposed, click to set yours"}
                      >
                        {PRIORITY_LABEL[p]}
                      </button>
                    ))
                  ) : (
                    <span className={`admin-pbacklog-pill on-${eff} u-cursor-default`}>{PRIORITY_LABEL[eff]}</span>
                  )}
                </span>
              </div>
              <div className="admin-pbacklog-body">
                ...
                <div className="admin-pbacklog-chips">
                  {(it.needs ?? []).map((n) => <span key={n} className="admin-pbacklog-chip">{n}</span>)}
                  {tok && <span className="admin-pbacklog-chip tok">est. {tok} Human Tokens</span>}
                  {it.source === "client" && (
                    <span className="admin-pbacklog-chip proposed">{it.status === "proposed" ? "your proposal, awaiting TeddyBed OS" : "your idea"}</span>
                  )}
                  {it.client_priority && it.client_priority !== it.edge8_priority && (
                    <span className="admin-pbacklog-chip mine">you changed from TeddyBed OS&apos;s {PRIORITY_LABEL[it.edge8_priority]}</span>
                  )}
                </div>
                {canPrioritize && it.client_priority && (
                  <div className="admin-pbacklog-hint">
                    You set this. Click the highlighted pill again to revert to TeddyBed OS&apos;s suggestion ({PRIORITY_LABEL[it.edge8_priority]}).
                  </div>
                )}
              </div>
            </div>
  ...
  return (
    <div className="admin-pbacklog">
      <p className="admin-pbacklog-intro">
        Your roadmap, section by section, as agreed with TeddyBed OS.{" "}
        {canPrioritize && (
        ...
        Human Token estimates are pre-research ranges (1 Human Token = 1 hour of TeddyBed OS expert time).
      </p>

      <div className="admin-pbacklog-counts">
        {BACKLOG_PRIORITIES.map((p) => (
          <span key={p} className="admin-pbacklog-count">{PRIORITY_LABEL[p]}: {counts[p]}</span>
        ))}
      </div>

      {err && <div className="admin-pbacklog-err">{err}</div>}

      <DragDropContext onDragEnd={onDragEnd}>
        {groups.map((g) => {
          const groupItems = ordered.filter((i) => i.group_key === g.key);
          return (
            <div key={g.key} className="admin-pbacklog-group">
              <div className="admin-pbacklog-group-head">
                {g.step_label && <span className="admin-pbacklog-step">{g.step_label}</span>}
                <span className="admin-pbacklog-group-title">{g.title}</span>
              </div>
              {g.intro && <div className="admin-pbacklog-group-intro">{g.intro}</div>}
              ...
              {companyId && canPropose && (proposeGroup === g.key ? (
                <div className="admin-pbacklog-propose">
                  <ProposeAssist
                    onDraft={(d) => {
                      setPTitle(d.title);
                      setPNote(d.note);
                      ...
                      setPHint(
                        d.groupKey !== g.key
                          ? `Tip: this might fit better under ${suggested?.title ?? d.groupKey}. You can propose it there instead, or send it here and TeddyBed OS will place it.`
                          : null,
                      );
                    }}
                  />
                  ...
                  {pHint && <div className="admin-pbacklog-hint u-mb-2">{pHint}</div>}
                  <div className="u-row">
                    <button
                      type="button"
                      className="admin-pbacklog-btn"
                    ...
                    <button type="button" className="admin-pbacklog-btn ghost" onClick={() => { setProposeGroup(null); setPTitle(""); setPNote(""); setPHint(null); }}>Cancel</button>
                  </div>
                </div>
              ) : (
                <button type="button" className="admin-pbacklog-link u-mt-2" onClick={() => setProposeGroup(g.key)}>
