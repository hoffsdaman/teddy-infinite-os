...
const STYLES = `
.cbp { --pri-now:var(--color-primary-blue); --pri-next:var(--admin-ok-strong); --pri-later:var(--admin-muted-ink); --pri-park:var(--admin-amber-ink); max-width: 880px; }
.cbp .cbp-intro { background:color-mix(in srgb, var(--admin-accent) 8%, transparent); border-radius:10px; padding:14px 16px; font-size:14px; margin:0 0 16px; }
.cbp .cbp-counts { display:flex; gap:6px; flex-wrap:wrap; margin:0 0 18px; }
.cbp .cbp-count { font-size:12px; font-weight:600; color:var(--admin-ink-2); padding:4px 11px; border-radius:99px; background:var(--admin-surface-2); }
...
.cbp .cbp-step { font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:.04em; padding:3px 9px; border-radius:99px; background:color-mix(in srgb, var(--admin-accent) 10%, transparent); color:var(--color-primary-blue); }
.cbp .cbp-group-title { font-weight:700; font-size:15px; }
.cbp .cbp-group-intro { color:var(--admin-ink-2); font-size:13px; margin:2px 0 12px; }
.cbp .cbp-item { border:1px solid var(--admin-line); border-radius:12px; padding:13px 15px 13px 12px; margin-bottom:9px; background:var(--admin-surface); display:flex; gap:10px; align-items:flex-start; }
.cbp .cbp-item.dragging { box-shadow:0 8px 24px color-mix(in srgb, var(--admin-ink) 14%, transparent); border-color:var(--color-primary-blue); }
.cbp .cbp-handle { flex:none; cursor:grab; color:var(--admin-faint); font-size:16px; line-height:1.2; padding:2px 2px 0; user-select:none; touch-action:none; }
...
.cbp .cbp-ref { flex:none; font-size:12px; font-weight:700; color:var(--color-primary-blue); background:color-mix(in srgb, var(--admin-accent) 10%, transparent); border-radius:6px; padding:3px 7px; }
.cbp .cbp-title { font-weight:650; font-size:14px; flex:1 1 220px; }
.cbp .cbp-pills { display:flex; gap:4px; flex-wrap:wrap; }
.cbp .cbp-pill { font-size:12px; font-weight:600; padding:4px 11px; border-radius:99px; border:1px solid var(--admin-line); background:var(--admin-surface); color:var(--admin-ink-2); cursor:pointer; font-family:inherit; }
...
.cbp .cbp-pill.on-next { background:color-mix(in srgb, var(--admin-ok-strong) 15%, transparent); border-color:var(--pri-next); color:var(--pri-next); }
.cbp .cbp-pill.on-later { background:var(--admin-surface-2); border-color:var(--admin-faint); color:var(--pri-later); }
.cbp .cbp-pill.on-park { background:var(--admin-amber-bg); border-color:var(--admin-warn-strong); color:var(--pri-park); }
.cbp .cbp-body { font-size:13px; margin-top:8px; color:var(--admin-ink); }
.cbp .cbp-body .k { color:var(--admin-ink-2); font-weight:600; }
.cbp .cbp-chips { display:flex; gap:6px; flex-wrap:wrap; margin-top:8px; align-items:center; }
.cbp .cbp-chip { font-size:11px; font-weight:600; color:var(--admin-ink-2); border:1px solid var(--admin-line-soft); border-radius:99px; padding:2px 9px; }
.cbp .cbp-chip.tok { color:var(--color-primary-blue); border-color:color-mix(in srgb, var(--admin-accent) 15%, transparent); background:color-mix(in srgb, var(--admin-accent) 8%, transparent); }
.cbp .cbp-chip.mine { color:var(--admin-ok-strong); border-color:color-mix(in srgb, var(--admin-ok-strong) 25%, transparent); background:color-mix(in srgb, var(--admin-ok-strong) 10%, transparent); }
.cbp .cbp-chip.proposed { color:var(--admin-amber-ink); border-color:var(--admin-warn-strong); background:var(--admin-amber-bg); }
.cbp .cbp-hint { font-size:11px; color:var(--admin-muted); margin-top:6px; }
.cbp .cbp-propose { margin-top:8px; border:1px dashed var(--admin-line-strong); border-radius:12px; padding:12px 14px; }
...
.cbp .cbp-btn.ghost { background:var(--admin-surface); color:var(--color-primary-blue); }
...
.cbp .cbp-err { color:var(--admin-err-strong); font-size:12px; margin-top:6px; }
