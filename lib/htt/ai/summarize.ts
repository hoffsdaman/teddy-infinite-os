...
/** Detect the provider from the key's prefix. Exported for tests. */
export function pickProvider(key: string): Provider {
  return key.startsWith("sk-or-") ? "openrouter" : "anthropic";
}

export function summaryModel(): string {
  const key = process.env.ANTHROPIC_API_KEY;
  if (key && pickProvider(key) === "openrouter") return OPENROUTER_MODEL;
...
async function generate(
  ...
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) {
    console.warn("[htt summaries] ANTHROPIC_API_KEY is not set; generation skipped");
    return null;
  }
