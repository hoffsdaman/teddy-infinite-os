...
export function validateBlogForPublish(
  ...
  if (emDashScope.includes("—")) errors.push("Contains an em dash (—). TeddyBed OS never uses em dashes.");

  // SEO floor: every post links into the rest of the blog (in the body, not the
  // FAQ), and the title tag must be crafted, not "{title} | TeddyBed OS Blog" filler.
  ...
  if (parsed.titleTag && /\|\s*TeddyBed OS Blog\s*$/i.test(parsed.titleTag)) {
    errors.push('Title tag is the generic "... | TeddyBed OS Blog" pattern. Write a keyword-led title tag.');
  }
